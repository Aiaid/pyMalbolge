...
- **Size**: programs with user-defined functions blow up noticeably on the `c` backend; switching to `--backend=direct` is typically 46-75% smaller. See the internal research log (private, not in this repo), §I, for the `.mb`-size cost model and optimization investigation.
