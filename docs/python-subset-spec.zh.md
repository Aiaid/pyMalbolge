...
| D5 | `getchar`/`putchar` 与 EOF | 无对应内建;需要 `sys.stdin`/`sys.stdout` | `getchar()` 读一个字符编码,`putchar(x)` 按 `x` 的编码写一个字符;**EOF 返回值未在 py2c 层规定**——`getchar()` 只是发射对下游 `getchar()` C 函数的调用,具体 EOF 语义由运行时(Malbolge20 参考实现:`A=59049`,见 `docs/findings.zh.md` B1)决定,py2c 本身不做任何 EOF 相关折算或校验 | 设计性偏差 + 文档缺口(v1 说明文档未提及 EOF 值,见"版本节"待办) |
...
- 2026-07-23:**批次一语法糖**在 `py2c.py`('c' 后端)与 `py2mg.py`
  ...
  `docs/findings.zh.md` A3 约束(标志走无分支累积 + 单次 SWITCH 模式)。
...
- **数组/运行时字符串**:`Subscript`/`Slice`/字符串**变量**仍全部拒绝
  ...
  机制解剖与 LOADI/STOREI 设计建议见 `docs/iwagane-arrays.zh.md`。
...
`docs/findings.zh.md` A4 的教训:不能只看中间产物"形态可疑"就下结论)。
