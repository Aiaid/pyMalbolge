...
对照 `docs/findings.zh.md` §C 记录的 hello20 "0.6 s":此次子进程整体耗时
0.89 s,差值主要是 Python 3.14 解释器自身启动 + import 开销(子进程整体
计时 vs. findings.md 未注明测量方式),量级一致,无回归。
...
(首尾两点直接算得 ≈1.50)。也就是说 `docs/findings.zh.md` §C 里"近线性、约
40s/MB"的描述**需要修正**——耗时增长明显快于线性(体积 4.54× → 耗时
...
## 建议追加进 findings.md 的条目草稿(未编辑 findings.md,供审阅后手工合并)
...
*(以上两条为草稿文本,是否、如何合并入 findings.md 由后续人工决定;
本报告本身不修改 findings.md。)*
